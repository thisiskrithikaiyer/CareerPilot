...
def _tool_estimate_funnel_params(args: dict, state: CrisisCoachState) -> dict:
    from crisiscoach.agents.background.target_recalibrator import (
...
def _handle_tool(name: str, args: dict, state: CrisisCoachState) -> str:
    try:
...
def _build_system(state: CrisisCoachState) -> str:
    ...
    if user_id:
        try:
            from crisiscoach.db.supabase import get_client
            from crisiscoach.db.belief_state import read as read_belief
    ...
    if not (tracking or {}).get("revision_mode"):
        try:
            from crisiscoach.agents.background.target_recalibrator import compute_initial_targets
...
def _extract_and_save_plan(user_id: str, response_text: str, revision_analytics: dict | None = None) -> str | None:
    ...
    try:
        from crisiscoach.db.supabase import get_client
        from crisiscoach.agents.background.role_curriculum import get_curriculum, detect_role_type
        ...
        try:
            from crisiscoach.db.belief_state import write as write_belief
            sb = get_client()
            belief_update: dict = {}
