"""Ingest role curriculum knowledge into the vector store (knowledge DB, not user DB)."""
import asyncio

COLLECTION = "role_curriculum"
...
ROLE_LABELS = {
    ...
    "backend_swe": "Backend Software Engineer",
    "frontend_swe": "Frontend Software Engineer",
}


def _build_chunks() -> list[tuple[str, dict]]:
    """Return (text, metadata) pairs for every curriculum entry."""
    chunks: list[tuple[str, dict]] = []

    for role_key, curriculum in CURRICULA.items():
        ...
        for entry in curriculum["leetcode_topics"]:
            ...
            text = (
                f"[{role_label}] LeetCode topic: {entry['topic']} "
                f"(difficulty: {entry['difficulty']}). "
                f"Key problems: {problems}. "
        ...
        for entry in curriculum["system_design_concepts"]:
            key_points = "; ".join(entry["key_points"])
            skills = ", ".join(entry.get("skills", []))
            text = (
                f"[{role_label}] System design concept: {entry['concept']}. "
        ...
        # ── Core concepts ──────────────────────────────────────────────────────
        for concept in curriculum["core_concepts"]:
            text = f"[{role_label}] Core concept: {concept}"
            chunks.append((text, {
                "role": role_key,
...
async def ingest_seed_data(force: bool = False):
    from careerpilot.db.vector_store import get_collection, embed_texts

    collection = get_collection(COLLECTION)

    if not force and collection.count() > 0:
        return {"ingested": 0, "collection": COLLECTION, "skipped": True}

    pairs = _build_chunks()
    texts = [t for t, _ in pairs]
    metas = [m for _, m in pairs]

    source = "careerpilot://seed/curriculum"
    ids = [f"{source}::chunk_{i}" for i in range(len(texts))]
    full_metas = [{**m, "source": source, "chunk_index": i} for i, m in enumerate(metas)]
    embeddings = await embed_texts(texts)
    collection.add(ids=ids, embeddings=embeddings, documents=texts, metadatas=full_metas)

    print(f"Ingested {len(texts)} curriculum chunks into '{COLLECTION}'.")
    return {"ingested": len(texts), "collection": COLLECTION}
