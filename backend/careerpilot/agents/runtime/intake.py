...
def _build_system(state: State, fields: dict, history: list[dict]) -> str:
    ...
    last_asked = _last_asked_field(history)
    if last_asked and not fields.get(last_asked):
        snippets.append(f"WARNING: You already asked about '{last_asked}' in your last message. DO NOT ask it again — wait for the user to respond.")
...
_GOAL_CONFIRM_PHRASES = {"yes, let's go", "let's do it", "let's go"}


def _check_goal_confirmed(history: list[dict]) -> bool:
    """True only if user confirmed a goal that was actually proposed in this conversation."""
    proposal_idx = None
    for i, msg in enumerate(history):
        if msg["role"] == "assistant":
            c = msg["content"].lower()
            if ("land " in c and "offer" in c) or "yes, let's go" in c:
                proposal_idx = i
    if proposal_idx is None:
        return False
    for msg in history[proposal_idx + 1:]:
        if msg["role"] == "user" and msg["content"].strip().lower() in _GOAL_CONFIRM_PHRASES:
            return True
    return False


def _last_asked_field(history: list[dict]) -> str | None:
    """Returns the field the agent asked about in its most recent message."""
    for msg in reversed(history):
        if msg["role"] == "assistant":
            c = msg["content"].lower()
            if "what kind of role" in c or "role are you targeting" in c:
                return "role"
            if "when do you need" in c or ("offer" in c and "timeline" in c):
                return "offer_timeline"
            if "leetcode" in c or "gut check" in c:
                return "leetcode_level"
            if "anything specific" in c or "factor in" in c:
                return "notes"
            return None
    return None
...
async def run(state: State) -> dict:
    ...
    if user_id:
        # Only persist fields confirmed this turn (not ones the frontend already sent back)
        newly_confirmed = {
            k: fields[k]
            for k in ("role", "offer_timeline", "leetcode_level")
            if fields.get(k) and not frontend_fields.get(k)
        }
        if newly_confirmed:
            _persist_intake_fields(user_id, newly_confirmed)

    system = _build_system(state, fields, history)
    resp = groq_complete(
        model=GROQ_MODEL,
        max_tokens=256,
        temperature=0,
    ...
    # Only allow completion if core fields are actually collected — never trust the model alone
    core_done = _core_fields_collected(fields)
    intake_done = core_done and goal_confirmed and notes_done
